...
def gibbs_sampler(dna: list[str], k: int, t: int, n: int) -> list[str]:
    ...
    for _ in range(1000):
        ...
        for _ in range(n):
            ignoredKmerIndex = chooseRandomKmerIndex(t) #choose random i 
            ...
            motifProbability = Motifs(profile, dna[ignoredKmerIndex], k) #find motif probabilty from the ignoredDNA
...
def Motifs (profile, string, k) -> dict:
    ...
    for i in range(len(string) - k + 1): 
        ...
        motifProbability[kmer] = probability #put kmer and probability in dict so that we can split later for randomly "choosing" given probaility 
