...
## Input 
`Gibby` can take HOMER `peaks.txt` files and `.bed` files as input. In addition, we require the appropriate genome assembly (which was used to generate the peak file) in `FASTA` format. 
